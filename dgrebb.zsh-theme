...
PROMPT='
%{$fg[yellow]%}👌 %{$reset_color%} %{$fg_bold[green]%}$(collapse_pwd)%{$reset_color%}
$(virtualenv_info)$(prompt_char)%{$reset_color%} '

RPROMPT='%{$fg_bold[yellow]%}$(git_time_since_commit)%{$reset_color%} $(git_custom_status)%{$reset_color%}%{$fg[white]%}$(python_version)'
